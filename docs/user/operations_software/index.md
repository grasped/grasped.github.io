...
This section shows the upload mission lists. The system is capable to apply filters by `status`, `receiving station` and `satellite`.
Inside the list we can also click the command to directly edit the selected command for ease of access.

#### Add Upload Mission

To upload a mission, there are certain number of parameters that are needed to be filled.

- name
- execution time
- Satellite (Defaults to Diwata 1)
- Receiving Station
- Status
- Remarks
...
Inside the table you can click the items inside the `ÌD` column, this will redirect you the view upload mission page. Where you can perform edits or delete the mission.

Inside this page, we can also copy the generated command or download it as CSV.
